        ...
        <div class="flex-1 flex items-center justify-end gap-1">
            ...
            <nav class="hidden lg:flex items-center gap-5 mr-4 text-[12px] font-semibold tracking-[0.12em] uppercase">
                <a href="{{ route('catalogo', ['nuevo' => 1]) }}"
                    class="{{ request()->boolean('nuevo') ? 'text-tinta border-b border-tinta pb-0.5' : 'text-gris' }} hover:text-tinta transition">Lo nuevo</a>
    ...
    <div class="p-5 space-y-7">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-2">
                <a href="{{ route('catalogo', ['nuevo' => 1]) }}" onclick="closeMenu()"
                ...
                @foreach ([['slug' => 'ropa', 'label' => 'Ropa'], ['slug' => 'calzado', 'label' => 'Calzado'], ['slug' => 'accesorios', 'label' => 'Accesorios']] as $item)
            ...
            @auth
                ...
                @if (auth()->user()->isAdmin())
                    <a href="{{ route('admin.dashboard') }}" onclick="closeMenu()"
                        class="flex items-center gap-2 py-2.5 text-sm font-semibold text-tinta hover:text-gris transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                        </svg>
                ...
                @if (!auth()->user()->isAdmin())
                    <a href="{{ route('favoritos.index') }}" onclick="closeMenu()"
                        class="flex items-center gap-2 py-2.5 text-sm font-medium hover:text-gris transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg>
                ...
                @endif
                <a href="{{ route('perfil.show') }}" onclick="closeMenu()"
    ...
    <div class="w-full px-4 sm:px-6 lg:px-10 py-4 sm:py-5">
        ...
        <div id="searchQuickLinks" class="mt-5">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('catalogo', ['nuevo' => 1]) }}" onclick="closeSearch()"
        ...
        if (productos.length && prodList && prodEl) {
            ...
            productos.forEach(p => {
                ...
                const ofertaBadge = p.oferta
                    ? `<span style="font-size:10px;background:#F4E6EA;color:#9B1D3A;border:1px solid #DFA8B4;padding:1px 7px;border-radius:999px;font-weight:600;letter-spacing:.05em;text-transform:uppercase;flex-shrink:0">Oferta</span>` : '';
                const origHtml = orig
